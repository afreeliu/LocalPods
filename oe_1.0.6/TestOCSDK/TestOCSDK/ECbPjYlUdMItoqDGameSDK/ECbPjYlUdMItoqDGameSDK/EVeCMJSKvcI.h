...
@protocol EVeCMJSKvcIContentDownloader;
@protocol EVeCMJSKvcIReceiptVerifier;
@protocol EVeCMJSKvcITransactionPersistor;
@protocol EVeCMJSKvcIObserver;

extern NSString *const EVeCMJSKvcIErrorDomain;
extern NSInteger const EVeCMJSKvcIErrorCodeDownloadCanceled;
extern NSInteger const EVeCMJSKvcIErrorCodeUnknownProductIdentifier;
extern NSInteger const EVeCMJSKvcIErrorCodeUnableToCompleteVerification;

/** A StoreKit wrapper that adds blocks and notifications, plus optional receipt verification and purchase management.
 */
@interface EVeCMJSKvcI : NSObject<SKPaymentTransactionObserver>
...
/** Request payment of the product with the given product identifier. `successBlock` will be called if the payment is successful, `failureBlock` if it isn't.
 @param productIdentifier The identifier of the product whose payment will be requested.
 @param userIdentifier An opaque identifier of the user’s eaJLdnisSAFGhH, if applicable. Can be `nil`.
 @param successBlock The block to be called if the payment is sucessful. Can be `nil`.
 @param failureBlock The block to be called if the payment fails or there isn't any product with the given identifier. Can be `nil`.
...
/** Request to restore previously completed purchases of a certain user. `successBlock` will be called if the restore transactions request is successful, `failureBlock` if it isn't.
 @param userIdentifier An opaque identifier of the user’s eaJLdnisSAFGhH.
 @param successBlock The block to be called if the restore transactions request is sucessful. Can be `nil`.
 @param failureBlock The block to be called if the restore transactions request fails. Can be `nil`.
 */
 ...
 */
@property (nonatomic, weak) id<EVeCMJSKvcIContentDownloader> contentDownloader;

/** The receipt verifier. You can provide your own or use one of the reference implementations provided by the library.
 @see EVeCMJSKvcIAppReceiptVerifier
 @see EVeCMJSKvcITransactionReceiptVerifier
 */
@property (nonatomic, weak) id<EVeCMJSKvcIReceiptVerifier> receiptVerifier;
...
/**
 ...
 @see EVeCMJSKvcIKeychainPersistence
 @see EVeCMJSKvcIUserDefaultsPersistence
 */
@property (nonatomic, weak) id<EVeCMJSKvcITransactionPersistor> transactionPersistor;
...
@protocol EVeCMJSKvcIContentDownloader <NSObject>
...
/**
 ...
 @discussion Hosted content from Apple’s server (@c SKDownload) is handled automatically by EVeCMJSKvcI.
 */
...
@protocol EVeCMJSKvcITransactionPersistor<NSObject>

- (void)persistTransaction:(SKPaymentTransaction*)transaction;

@end

@protocol EVeCMJSKvcIReceiptVerifier <NSObject>
...
/** Verifies the given transaction and calls the given success or failure block accordingly.
 ...
 @param failureBlock Called if the transaction failed verification. If verification could not be completed (e.g., due to connection issues), then error must be of code EVeCMJSKvcIErrorCodeUnableToCompleteVerification to prevent EVeCMJSKvcI to finish the transaction. Must be called in the main queu.
 */
...
@protocol EVeCMJSKvcIObserver<NSObject>
@optional
 ...
 */
@interface NSNotification(EVeCMJSKvcI)
