...
NSString *const DUjDbacgMfiohnASYErrorDomain = @"net.robotmedia.store";
NSInteger const DUjDbacgMfiohnASYErrorCodeDownloadCanceled = 300;
NSInteger const DUjDbacgMfiohnASYErrorCodeUnknownProductIdentifier = 100;
NSInteger const DUjDbacgMfiohnASYErrorCodeUnableToCompleteVerification = 200;
...
NSString* const DUjDbacgMfiohnASYNotificationInvalidProductIdentifiers = @"invalidProductIdentifiers";
NSString* const DUjDbacgMfiohnASYNotificationDownloadProgress = @"downloadProgress";
NSString* const DUjDbacgMfiohnASYNotificationProductIdentifier = @"productIdentifier";
NSString* const DUjDbacgMfiohnASYNotificationProducts = @"products";
NSString* const DUjDbacgMfiohnASYNotificationStoreDownload = @"storeDownload";
NSString* const DUjDbacgMfiohnASYNotificationStoreError = @"storeError";
NSString* const DUjDbacgMfiohnASYNotificationStoreReceipt = @"storeReceipt";
NSString* const DUjDbacgMfiohnASYNotificationTransaction = @"transaction";
NSString* const DUjDbacgMfiohnASYNotificationTransactions = @"transactions";

#if DEBUG
#define DUjDbacgMfiohnASYLog(...) NSLog(@"DUjDbacgMfiohnASY: %@", [NSString stringWithFormat:__VA_ARGS__]);
#else
#define DUjDbacgMfiohnASYLog(...)
#endif
...
typedef void (^DUjDbacgMfiohnASYFailureBlock)(NSError *error);
typedef void (^DUjDbacgMfiohnASYSuccessBlock)(void);

@implementation NSNotification(DUjDbacgMfiohnASY)

- (float)rm_downloadProgress
{
    return [self.userInfo[DUjDbacgMfiohnASYNotificationDownloadProgress] floatValue];
}

- (NSArray*)rm_invalidProductIdentifiers
{
    return (self.userInfo)[DUjDbacgMfiohnASYNotificationInvalidProductIdentifiers];
}

- (NSString*)rm_productIdentifier
{
    return (self.userInfo)[DUjDbacgMfiohnASYNotificationProductIdentifier];
}

- (NSArray*)rm_products
{
    return (self.userInfo)[DUjDbacgMfiohnASYNotificationProducts];
}

- (SKDownload*)rm_storeDownload
{
    return (self.userInfo)[DUjDbacgMfiohnASYNotificationStoreDownload];
}

- (NSError*)rm_storeError
{
    return (self.userInfo)[DUjDbacgMfiohnASYNotificationStoreError];
}

- (SKPaymentTransaction*)rm_transaction
{
    return (self.userInfo)[DUjDbacgMfiohnASYNotificationTransaction];
}

- (NSArray*)rm_transactions {
    return (self.userInfo)[DUjDbacgMfiohnASYNotificationTransactions];
}
...
@property (nonatomic, weak) DUjDbacgMfiohnASY *store;
...
@interface DUjDbacgMfiohnASY() <SKRequestDelegate>

@end

@implementation DUjDbacgMfiohnASY {
...
+ (DUjDbacgMfiohnASY *)defaultStore
{
    static DUjDbacgMfiohnASY *sharedInstance = nil;
           ...
           failure:(void (^)(SKPaymentTransaction *transaction, NSError *error))failureBlock
{
    ...
    if (product == nil)
    {
        DUjDbacgMfiohnASYLog(@"unknown product id %@", productIdentifier)
        if (failureBlock != nil)
        {
            NSError *error = [NSError errorWithDomain:DUjDbacgMfiohnASYErrorDomain code:DUjDbacgMfiohnASYErrorCodeUnknownProductIdentifier userInfo:@{NSLocalizedDescriptionKey: NSLocalizedStringFromTable(@"Unknown product identifier", @"DUjDbacgMfiohnASY", @"Error description")}];
...
- (void)refreshReceiptOnSuccess:(DUjDbacgMfiohnASYSuccessBlock)successBlock
                        failure:(DUjDbacgMfiohnASYFailureBlock)failureBlock
{
...
- (void)addStoreObserver:(id<DUjDbacgMfiohnASYObserver>)observer
{
...
- (void)removeStoreObserver:(id<DUjDbacgMfiohnASYObserver>)observer
{
...
- (void)addStoreObserver:(id<DUjDbacgMfiohnASYObserver>)observer selector:(SEL)aSelector notificationName:(NSString*)notificationName
{
...
{
    DUjDbacgMfiohnASYLog(@"restore transactions finished");
...
{
    DUjDbacgMfiohnASYLog(@"restored transactions failed with error %@", error.debugDescription);
    ...
    { // error might be nil (e.g., on airplane mode)
        userInfo = @{DUjDbacgMfiohnASYNotificationStoreError: error};
    }
...
- (void)didCancelDownload:(SKDownload*)download queue:(SKPaymentQueue*)queue
{
    ...
    DUjDbacgMfiohnASYLog(@"download %@ for product %@ canceled", download.contentIdentifier, download.transaction.payment.productIdentifier);

    [self postNotificationWithName:RMSKDownloadCanceled download:download userInfoExtras:nil];

    NSError *error = [NSError errorWithDomain:DUjDbacgMfiohnASYErrorDomain code:DUjDbacgMfiohnASYErrorCodeDownloadCanceled userInfo:@{NSLocalizedDescriptionKey: NSLocalizedStringFromTable(@"Download canceled", @"DUjDbacgMfiohnASY", @"Error description")}];
...
- (void)didFailDownload:(SKDownload*)download queue:(SKPaymentQueue*)queue
{
    ...
    SKPaymentTransaction *transaction = download.transaction;
    DUjDbacgMfiohnASYLog(@"download %@ for product %@ failed with error %@", download.contentIdentifier, transaction.payment.productIdentifier, error.debugDescription);

    NSDictionary *extras = error ? @{DUjDbacgMfiohnASYNotificationStoreError : error} : nil;
...
{
    SKPaymentTransaction *transaction = download.transaction;
    DUjDbacgMfiohnASYLog(@"download %@ for product %@ finished", download.contentIdentifier, transaction.payment.productIdentifier);
...
{
    DUjDbacgMfiohnASYLog(@"download %@ for product %@ paused", download.contentIdentifier, download.transaction.payment.productIdentifier);
...
{
    DUjDbacgMfiohnASYLog(@"download %@ for product %@ updated", download.contentIdentifier, download.transaction.payment.productIdentifier);
    NSDictionary *extras = @{DUjDbacgMfiohnASYNotificationDownloadProgress : @(download.progress)};
...
{
    DUjDbacgMfiohnASYLog(@"transaction purchased with product %@", transaction.payment.productIdentifier);
    ...
    {
        DUjDbacgMfiohnASYLog(@"WARNING: no receipt verification");
        [self didVerifyTransaction:transaction queue:queue];
    }
}
...
- (void)didFailTransaction:(SKPaymentTransaction *)transaction queue:(SKPaymentQueue*)queue error:(NSError*)error
{
    ...
    DUjDbacgMfiohnASYLog(@"transaction failed with product %@ and error %@", productIdentifier, error.debugDescription);
    
    if (error.code != DUjDbacgMfiohnASYErrorCodeUnableToCompleteVerification)
...
{
    DUjDbacgMfiohnASYLog(@"transaction restored with product %@", transaction.originalTransaction.payment.productIdentifier);
...
- (void)didVerifyTransaction:(SKPaymentTransaction *)transaction queue:(SKPaymentQueue*)queue
{
    if (self.contentDownloader != nil)
    {
        [self.contentDownloader downloadContentForTransaction:transaction success:^{
            ...
            NSDictionary *extras = @{DUjDbacgMfiohnASYNotificationDownloadProgress : @(progress)};
...
- (void)didDownloadSelfHostedContentForTransaction:(SKPaymentTransaction *)transaction queue:(SKPaymentQueue*)queue
{
    ...
    {
        DUjDbacgMfiohnASYLog(@"starting downloads for product %@ started", transaction.payment.productIdentifier);
...
- (void)notifyRestoreTransactionFinishedIfApplicableAfterTransaction:(SKPaymentTransaction*)transaction
{
    ...
    { // Wait until all restored transations have been verified
        ...
        NSDictionary *userInfo = @{ DUjDbacgMfiohnASYNotificationTransactions : restoredTransactions };
...
{
    DUjDbacgMfiohnASYLog(@"refresh receipt finished");
...
{
    DUjDbacgMfiohnASYLog(@"refresh receipt failed with error %@", error.debugDescription);
    _refreshReceiptRequest = nil;
...
- (void)postNotificationWithName:(NSString*)notificationName download:(SKDownload*)download userInfoExtras:(NSDictionary*)extras
{
    ...
    mutableExtras[DUjDbacgMfiohnASYNotificationStoreDownload] = download;
...
- (void)postNotificationWithName:(NSString*)notificationName transaction:(SKPaymentTransaction*)transaction userInfoExtras:(NSDictionary*)extras
{
    ...
    userInfo[DUjDbacgMfiohnASYNotificationTransaction] = transaction;
    userInfo[DUjDbacgMfiohnASYNotificationProductIdentifier] = productIdentifier;
    if (extras)
    {
...
{
    DUjDbacgMfiohnASYLog(@"products request received response");
    ...
    {
        DUjDbacgMfiohnASYLog(@"received product with id %@", product.productIdentifier);
    ...
    [invalidProductIdentifiers enumerateObjectsUsingBlock:^(NSString *invalid, NSUInteger idx, BOOL *stop) {
        DUjDbacgMfiohnASYLog(@"invalid product with id %@", invalid);
    }];
    ...
    NSDictionary *userInfo = @{DUjDbacgMfiohnASYNotificationProducts: products, DUjDbacgMfiohnASYNotificationInvalidProductIdentifiers: invalidProductIdentifiers};
...
- (void)request:(SKRequest *)request didFailWithError:(NSError *)error
{
    DUjDbacgMfiohnASYLog(@"products request failed with error %@", error.debugDescription);
